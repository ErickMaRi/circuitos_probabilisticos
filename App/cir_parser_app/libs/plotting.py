...
def plot_simulation_results(simulation_results):
    ...
    for file_name, plot in simulation_results.items():
        ...
        voltage_vector = np.array(plot[str(list(plot.keys())[1])])
        # Creamos el plot
        ...
        plt.xlabel('Time (s)')
        plt.ylabel(str(list(plot.keys())[1]))
...
def estimate_distribution(simulation_results, num_timesteps, dist_type='normal', interpolation_method='linear'):
    ...
    min_time = min(min(result['time']) for result in simulation_results.values())
    max_time = max(max(result['time']) for result in simulation_results.values())
    ...
    for circuit_name, circuit_results in simulation_results.items():
        ...
        f = interpolate.interp1d(times, values, kind=interpolation_method, fill_value='extrapolate')
...
def plot_distributions(fitted_distributions, num_samples=1000, n = 10):
    data = []
...
def plot_density(simulation_results, num_timesteps, num_bins=200, interpolation_method='linear'):
    ...
    plt.hist2d(time_data, flattened_data, bins=num_bins, density=True, cmap='plasma', norm=LogNorm())
    plt.xlabel('Time')
    plt.ylabel('Value')
