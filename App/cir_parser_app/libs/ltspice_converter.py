...
def LTSpice_to_float(string):
    ...
    try:
        ...
        if multiplier:
            string = re.sub(r'[\D\s]', '', string) #Limpiamos el string
            if '.' in string:
                parte_entera, parte_decimal = string.split('.') #Separamos respecto al punto
                string = (float(parte_entera)*multiplier) + \
                (float(parte_decimal)*pow(10, -len(parte_decimal)))*multiplier #Ensamblamos el valor float /
                # sumando la parte entera y la decimal.
            else:
                string = float(string)*multiplier
